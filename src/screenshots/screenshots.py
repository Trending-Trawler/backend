...
def login():
    try:
        reddit = praw.Reddit(client_id="Bd_J5_6O-Ym8RkkwjFRisw",
                             client_secret="-EZvGKTQzR6gS_MJzVlE6tR-t30nKQ",
...
def get_thread(reddit: Reddit, url):
...
def get_screenshots_of_reddit_posts(reddit_thread, reddit_comments, screenshot_num: int, theme="dark"):
    ...
    with sync_playwright() as p:
        ...
        if theme == "dark":
            cookie_file = open(
                ".\data\cookie-dark-mode.json", encoding="utf-8"
            )
...
my_reddit = login()
thread = get_thread(my_reddit, "https://www.reddit.com/r/Overwatch/comments/11e4o67/1f_shatter_block_diamond_5_pc/")
# print(thread)
comments = get_comments(thread)
# print(comments)
get_screenshots_of_reddit_posts(thread, comments, 10)
